...
def semileptonic_boosted(events, params, year, sample, **kwargs):
    ...
    mask = (
        ...
        & (events.nBJetGood >= params["nbjet"])
        & (events.MET.pt > params["met"])
        & (events.nFatJetGood >= params["nfatjet"])
        & (events.nLightJetGood >= params["nlightjet"])
    )
...
def signal_region(events, params, year, sample, **kwargs):

    mask = ak.num(events.BFatJetGood) >= 1

    # Pad None values with False
    return ak.where(ak.is_none(mask), False, mask)

def control_region(events, params, year, sample, **kwargs):

    mask = ak.num(events.BFatJetGood) == 0
